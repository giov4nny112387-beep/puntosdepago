        ...
        function _evaluarPerfilDia(colIdx, esDomingoDia, bateria) {
            let cob = _cobCajasDia(colIdx, esDomingoDia);
        ...
        function renderLineaTiempoCobertura() {
            ...
            ORDEN_ROL.forEach(cat => {
                ...
                horas.forEach(h => {
                    html += `<th style="background:#34495e; color:#fff; padding:5px 0; min-width:${anchoCol}px;">${h}</th>`;
                });
                html += `</tr></thead><tbody>`;
                ...
                gente.forEach((p, ri) => {
                    ...
                    horas.forEach((h, hi) => {
                        ...
                        if (e1 === 'off' && e2 === 'off') {
                            ...
                            cel = `<td style="padding:0; border:1px solid #eee; background:${colorBarra};"></td>`;
                        } else {
            ...
            if (bateria > 0) {
                ...
                if (ev.fallos.length) {
                    let detalle = ev.fallos.map(f =>
                        `<b>${f.h}:00</b> → ${f.n}/${bateria} cajas (${Math.round(f.pct * 100)}%): faltan ${f.faltan} para el ${Math.round(f.obj * 100)}% [${f.txt}]`);
                    let notaExtra = ev.falloUltimo
                        ? `<div style="font-size:12px; color:#7b241c; margin-top:8px; padding:8px 10px; background:#fbeee6; border-left:4px solid #e67e22; border-radius:4px;">
                             ⏱️ <b>ÚLTIMO TURNO:</b> con turnos que cubren el pico de 11:00-13:00 nadie alcanza también el cierre de las 21:30.
                             La cobertura del último turno debe cubrirse con <b>HORAS EXTRA de personas que salen un poco antes</b>.</div>`
                        : '';
                    bannerDomingo = `<div style="margin:0 0 14px 0; padding:12px 16px; background:#fdecea; border:1px solid #e74c3c; border-left:6px solid #c0392b; border-radius:8px;">
                        <div style="font-weight:bold; color:#c0392b; font-size:13px; margin-bottom:6px;">🔴 ${nombreDia}: no se cumple el perfil de cobertura de la batería (${bateria} cajas) — meta: ${_descPerfil()}${!_tablaAnclasTienda() && esDomingoDia ? ' · ≥80% en la tarde' : ''}</div>
                        <div style="font-size:12px; color:#7b241c;">Sumando Cajeros + Emergentes (sin personas con X):<br>${detalle.join('<br>')}</div>
                        ${notaExtra}
                        <div style="font-size:11px; color:#7f8c8d; margin-top:6px;">Razones posibles: personal insuficiente ese día (LIBREs/COMP/VC concentrados), personas con restricción X que no suman, o faltan emergentes. Revisa la matriz o añade refuerzos. El resumen de TODOS los días está en «🛡️ Cumplimiento de Cobertura».</div>
                    </div>`;
                } else {
            ...
            cont.innerHTML = infoDia + bannerDomingo + (html || '<div style="color:#7f8c8d; padding:10px;">Sin personal con turno horario este día.</div>');
        }
        ...
        const PLANTA_IDEAL_CFG = {
            'AKB30': {
                ...
                cintaDomingo: [           // Domingo — SIEMPRE 8h — 33 cupos (10/4/5/14, igual que el generador)
                    ['8i8-17',      10, '12:30-13:30'], // 8:00-17:00  (con almuerzo)
                    ['8i10.3-19.3',  4, '14:30-15:30'], // 10:30-19:30 (con almuerzo)
        ...
        function renderPlantaIdeal() {
            ...
            const cfg = PLANTA_IDEAL_CFG[tienda];
            if (!cfg) {
